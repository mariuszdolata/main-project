...
//		firstCompanyRepository.printAllCompanies();
		firstCompanyRepository.findCompanyType();

//		secondCompanyRepository.printAllCompanies();
		secondCompanyRepository.findCompanyType();
		//inicjalizacja tabeli wyników uzale¿niona od liczby stringów w repozytoriach
...
//				System.out.println("zamieniono wartoœci " + first + "|| " + second);
				if (first != null && second != null) {
//					System.out.println("Wejscie w if");
					counter++;
					try {
//						distanceTable[i][j] = lew.levenshteinDistance(first, second);
						double score=levenshtein.distance(firstCompanyRepository.getCompanies().get(i).getTidyCompanyName(),secondCompanyRepository.getCompanies().get(j).getTidyCompanyName());
						if(score<120)distanceTable[i][j] = (byte)score;
						else distanceTable[i][j] = 120;
//						System.out.println("Wejscie w try");
					} catch (Exception e) {
//						System.err.println("M: ubable to compute levensteinDistance");
						e.printStackTrace();
					}
				}
		...
		for (int i = 0; i < firstCompanyRepository.getCompanies().size(); i++) {
			for (int j = 0; j < secondCompanyRepository.getCompanies().size(); j++) {
				if(this.distanceTable[i][j]==0) System.out.println("["+this.distanceTable[i][j]+"]["+i+"]["+j+"]["+firstCompanyRepository.getCompanies().get(i).getTidyCompanyName()+"]["+secondCompanyRepository.getCompanies().get(j).getTidyCompanyName()+"]");
